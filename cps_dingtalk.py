...
class connectDingTalk():
    ...
    def getCheckinfo(self,checklist):
        ...
        checkDateTo = datetime.datetime.strftime(datetime.date.today(),"%Y-%m-%d 23:30:00")
        # 考勤列表暂时写死，需要从userlist获取
        # checkUserIdList = ['022935515930625665','186830624439954676']
        checkUserIdList = checklist #参数有问题要改
        print("checklist:")
        print(checklist)
        ...
        # req.userIdList = ['022935515930625665','186830624439954676']#参数必须要加双引号 要改
        ...
        try:
            ...
            print("拿到的 打卡API响应：")
            print(resp)
            resultlist.append(resp.get("recordresult"))
            ...
            for i in range(len(resultlist)):
                for j in range(len(resultlist[i])):
                    checkResultInfo.append(resultlist[i][j])
...
run.loopCheckInfo()
# 下面的无需单独执行
#run.getCheckinfo() #获取考勤信息API 并调用updateCheckInfoToDB插入数据库 已在loopcheckinfo中调用
